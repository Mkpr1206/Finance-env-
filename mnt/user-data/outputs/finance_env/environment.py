"""
environment.py — root-level shim so inference.py imports work unchanged.
Re-exports everything from server.environment plus the original Pydantic models.
"""
...
class PersonalFinanceEnv:
    ...
    def reset(self) -> Observation:
        ...
        self._txns = [
            Transaction(**{**t, "category": ExpenseCategory(t["category"]) if t["category"] else None})
            for t in templates
        ]
        if self._cfg["shock"]:
    ...
    def step(self, action: Action) -> tuple[Observation, Reward, bool, dict]:
        ...
        return obs, reward, self._done, dict(day=self._day, cash=round(self._cash, 2),
            savings_rate=round(self._savings_rate(), 4), remaining_items=remaining)

    def state(self) -> dict[str, Any]:
        return dict(task_id=self.task_id, day=self._day, cash=self._cash,
            savings=self._savings, investments=self._investments, debt=self._debt,
            savings_rate=self._savings_rate(),
            buckets={k: v.model_dump() for k, v in self._buckets.items()},
            action_log=self._action_log, done=self._done)
    ...
    def _apply(self, action: Action) -> Reward:
        ...
        h = handlers.get(action.action_type)
        return h(action) if h else Reward(value=-0.05, reason="Unknown action")

    def _categorize(self, a):
        txn = next((t for t in self._txns if t.id == a.transaction_id and t.category is None and not t.pending), None)
        if txn is None: return Reward(value=-0.1, reason="Not found")
        if a.category is None: return Reward(value=-0.05, reason="No category")
        correct = self._guess(txn)
        txn.category = a.category
        b = a.category.value
        if b in self._buckets: self._buckets[b].spent += txn.amount
        self._cash -= txn.amount
        v = round(max(-1.0, min(1.0, (0.5 if a.category == correct else -0.1) + (0.1 if len(a.rationale) > 20 else 0))), 3)
        return Reward(value=v, reason=f"Cat={a.category} correct={correct}")

    def _approve(self, a):
        txn = next((t for t in self._txns if t.id == a.transaction_id and t.pending), None)
        if txn is None: return Reward(value=-0.1, reason="Not found")
        if txn.category is None: txn.category = self._guess(txn)
        b = txn.category.value
        if b in self._buckets: self._buckets[b].spent += txn.amount
        self._cash -= txn.amount
        txn.pending = False
        s = 0.3 if txn.essential else (-0.3 if self._buckets.get(b) and self._buckets[b].utilization > 0.9 else (-0.2 if self._cash < 500 else 0.1))
        return Reward(value=round(max(-1.0, min(1.0, s + (0.1 if len(a.rationale) > 20 else 0))), 3))
    ...
    def _reject(self, a):
        ...
        s = -0.5 if txn.essential else (0.4 + (0.2 if self._cash < 800 else 0))
        return Reward(value=round(max(-1.0, min(1.0, s + (0.1 if len(a.rationale) > 20 else 0))), 3))

    def _allocate(self, a):
        fb, tb = self._buckets.get(a.from_bucket or ""), self._buckets.get(a.to_bucket or "")
        if not fb or not tb or not a.amount: return Reward(value=-0.1, reason="Invalid")
        if a.amount > fb.remaining: return Reward(value=-0.1, reason="Insufficient")
        fb.allocated -= a.amount; tb.allocated += a.amount
        return Reward(value=round(min(1.0, 0.2 + (0.2 if tb.utilization > 0.85 else 0) + (0.1 if len(a.rationale) > 20 else 0)), 3))

    def _invest(self, a):
        amt = a.amount or 0.0
        if amt <= 0: return Reward(value=-0.05)
        if amt > self._cash - 300: return Reward(value=-0.2, reason="Breaches buffer")
        self._cash -= amt; self._investments += amt; self._invested_this_month += amt
        return Reward(value=round(max(-1.0, min(1.0, min(0.5, amt/500) + (-0.15 if self._debt > 2000 and amt > 200 else 0) + (0.1 if len(a.rationale) > 20 else 0))), 3))

    def _pay_debt(self, a):
        amt = a.amount or 0.0
        if amt <= 0: return Reward(value=-0.05)
        if amt > self._cash - 300: return Reward(value=-0.2, reason="Breaches buffer")
        actual = min(amt, self._debt); self._cash -= actual; self._debt -= actual
        return Reward(value=round(min(1.0, min(0.5, actual/300) + (0.1 if len(a.rationale) > 20 else 0)), 3))

    def _guess(self, txn) -> ExpenseCategory:
    ...
    def _savings_rate(self):
        return round(max(0.0, min(1.0, (self._savings + self._invested_this_month + self._rejected_total) / self.MONTHLY_INCOME)), 4)
    ...
    def _obs(self):
        ...
        return Observation(day=self._day, monthly_income=self.MONTHLY_INCOME,
            cash_balance=round(self._cash, 2), savings_balance=round(self._savings, 2),
            investment_balance=round(self._investments, 2), debt_balance=round(self._debt, 2),
            budget_buckets=self._buckets, pending_transactions=pending,
            recent_transactions=recent, uncategorized_transactions=uncategorized,
            savings_rate=self._savings_rate(), month_complete=self._done)
